      ...
      if (response.statusCode == 200) {
        ...
        for (int i = 0; i < data.length; i++) {
          try {
            ...
            errorCount++;
            final itemData = data[i];
            ['harga', 'berat', 'garansi_berlaku', 'tanggal_garansi']
                .forEach((field) {
        ...
        print(
            '✅ Successfully parsed ${barangList.length} out of ${data.length} items');
        if (errorCount > 0) {
        ...
        print(
            'Failed to load barang: ${response.statusCode} - ${response.body}');
